  ...
  return (
    <div className="inventory-table-wrapper">
      <TableComponents
        ...
        colSpanColumns={true}
        renderRow={(medicine: Medicine) => (
          <React.Fragment key={medicine.id}>
           
            <TableRow
              className={`cursor-pointer transition-all duration-200 ${getRowClassName(medicine)}`}
              onClick={(e) => {
                if (!(e.target as HTMLElement).closest("button")) {
                  handleRowExpand(medicine)
                }
              }}
            >
              <TableCell className="p-3 border border-gray-300">
                <Button variant="ghost" size="sm" onClick={() => handleRowExpand(medicine)} className="text-gray-600">
                  {expandedRow === medicine.id ? <FaChevronUp /> : <FaChevronDown />}
                </Button>
              </TableCell>
              <TableCell className="p-3 border border-gray-300">{medicine.medicina}</TableCell>
              <TableCell className="p-3 border border-gray-300">{medicine.cantidad} unidades</TableCell>
              <TableCell className="p-3 border border-gray-300">{formatDate(medicine.fechaLlegada)}</TableCell>
              <TableCell
                className={`p-3 border border-gray-300 ${
                  isExpired(medicine.fechaExpiracion)
                    ? "expired-date"
              ...
              >
                {formatDate(medicine.fechaExpiracion)}
              </TableCell>
              <TableCell className="p-3 border border-gray-300">
                <Button variant="ghost" size="sm" onClick={() => onEdit(medicine)} className="text-blue-600">
                  <FaRegEdit />
                </Button>
              </TableCell>
              <TableCell className="p-3 border border-gray-300">
                <Button variant="ghost" size="sm" onClick={() => onDelete(medicine.id)} className="text-red-600">
                  <FaRegTrashAlt />
                </Button>
              </TableCell>
            </TableRow>
            ...
            {expandedRow === medicine.id && additionalData[medicine.id] && (
